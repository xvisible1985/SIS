# Account Page — Frontend Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the `/account` page with 4 tabs (Profile, Billing, Integrations, Referrals) wired to the backend endpoints.

**Architecture:** Single `AccountPage.tsx` component manages tab state and data fetching. Each tab renders its own section JSX inline. `frontend/src/api/account.ts` wraps all API calls using the existing `apiClient` (axios, auto-injects `Authorization: Bearer`). Layout integration via `App.tsx` route and `Layout.tsx` settings button.

**Tech Stack:** React 18 + TypeScript, Tailwind CSS, `apiClient` from `../api/client`, lucide-react icons

---

## File Structure

- **Create:** `frontend/src/api/account.ts` — typed API wrappers for all `/account/*` endpoints
- **Create:** `frontend/src/pages/AccountPage.tsx` — page with 4 tabs
- **Modify:** `frontend/src/App.tsx` — add `/account` route
- **Modify:** `frontend/src/components/Layout.tsx` — wire `onOpenSettings` to navigate to `/account`
...
Expected: no errors related to `account.ts`

- [ ] **Step 3: Commit**

```bash
git add frontend/src/api/account.ts
git commit -m "feat: account API client"
```

---

### Task 7: AccountPage — Skeleton + Profile Tab

**Files:**
- Create: `frontend/src/pages/AccountPage.tsx`

- [ ] **Step 1: Create AccountPage with tab skeleton and Profile tab**
...
Expected: no errors related to `AccountPage.tsx`

- [ ] **Step 3: Commit**

```bash
git add frontend/src/pages/AccountPage.tsx
git commit -m "feat: AccountPage with Profile, Billing, Integrations, Referrals tabs"
```
...
Inside the nested `<Routes>` (inside `<Layout>`), add after the `admin` route:

```tsx
<Route path="account" element={<AccountPage />} />
```

Full block context (show where to insert — after the admin route):
```tsx
...
- [ ] **Step 2: Update onOpenSettings in Layout.tsx**

In `frontend/src/components/Layout.tsx`, change:
...
1. Open the app and click the Settings gear icon (bottom of sidebar, on the user card)
2. Should navigate to `/account`
3. Profile tab loads showing email
4. Switch to each tab — no crashes
5. Billing tab shows balance card and plan name
6. Back to terminal: check network tab in DevTools — `GET /account/profile` returns 200

- [ ] **Step 5: Commit**

```bash
git add frontend/src/App.tsx frontend/src/components/Layout.tsx
git commit -m "feat: wire /account route and sidebar settings navigation"
```
