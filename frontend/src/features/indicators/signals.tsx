...
export const SIGNALS: SignalDef<any>[] = [
  ...
  },
  {
    ...
    defaults: { min_leverage: 0 },
    params: [
      { kind: 'number', key: 'min_leverage', label: 'Мин. плечо биржи', hint: 'Монета допускается, только если биржа разрешает плечо ≥ этого значения. 0 — без ограничения.', min: 0, step: 1, decimals: 0 },
    ],
    formula: (p: any) => <>{name('Плечо биржи')} {op('≥')} {num(`${p.min_leverage}x`)}</>,
    compute: (): SignalState => 'neutral',
  },
  {
