...
/** Human-readable labels for segmented param option values. */
const SEGMENTED_LABELS: Record<string, string> = {
  trend:    'Тренд',
  flat:     'Боковик',
  bull:     'Лонг',
  bear:     'Шорт',
...
export function SignalPickerField({ configs, onChange, onAutoSave, direction, liveSignal, priorityKey, onPriorityChange }: {
  configs: SignalConfig[]
  onChange: (configs: SignalConfig[]) => void
  onAutoSave?: (configs: SignalConfig[]) => void
  direction?: string
  liveSignal?: LiveSignalState
  /** Currently selected priority signal key ("name" or "name:tf") */
  priorityKey?: string
  /** Called when user clicks the priority star; passes undefined to clear */
  onPriorityChange?: (key: string | undefined) => void
}) {
  ...
  function handleApplyEdit() {
    ...
    const newConfigs = configs.map((c, i) => i === editingIdx ? { ...c, params } : c)
    onChange(newConfigs)
    ...
    <div className="flex flex-col gap-1.5">
        ...
        return (
          <div key={idx} className="flex items-center gap-2 min-w-0">
            ...
            >
              ...
              {onPriorityChange && (() => {
                const pKey = sigPriorityKey(sc)
                const isP = priorityKey === pKey
                return (
                  <button
                    onClick={() => onPriorityChange(isP ? undefined : pKey)}
                    className="shrink-0 w-7 h-7 inline-flex items-center justify-center rounded-[6px] transition-colors"
                    style={isP
                      ? { color: '#f59e0b' }
                      : { color: 'var(--sl-500, #64748b)' }
                    }
                    title={isP ? 'Снять приоритет' : 'Сделать приоритетным'}
                  >
                    {isP
                      ? <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                      : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                    }
                  </button>
                )
              })()}
      ...
      <div ref={ref} className="relative">
        ...
        {open && ReactDOM.createPortal(
          <div ref={dropRef} style={dropStyle} className="bg-[#0d1628] border border-white/[.10] rounded-xl shadow-2xl overflow-hidden">
            {step === 'list' ? (
...
// ─── SignalGateField ──────────────────────────────────────────────────────────
// Multi-signal gate for TP/SL. Supports multiple signals (AND logic) with
// per-signal param editing, and a shared direction selector (Buy/Sell/Авто).
// Follows the same UX pattern as SignalPickerField.

export function SignalGateField({
  configs,
  dir,
  onChange,
  buttonLabel,
  ...
  configs: SignalConfig[]
  dir: 'buy' | 'sell' | null
  onChange: (configs: SignalConfig[], dir: 'buy' | 'sell' | null) => void
  buttonLabel?: string
}) {
  const [open, setOpen] = useState(false)
  const [step, setStep] = useState<'list' | 'config' | 'edit'>('list')
  ...
  const available = allowedLoading
    ? []
    : allPickable.filter(s => allowedIds === null || allowedIds.has(s.id))
  ...
  function computeDropStyle() {
    ...
    const above = spaceBelow < 320 && r.top > 320
    setDropStyle({
      position: 'fixed', zIndex: 9999, width: dropW,
      left: Math.min(r.left, window.innerWidth - dropW - 8),
  ...
  function handleAdd() {
    if (!picked) return
    onChange([...configs, { name: picked.id, params }], dir)
    setOpen(false); setStep('list'); setPicked(null); setEditingIdx(null)
  }
  ...
  function handleApplyEdit() {
    if (editingIdx == null) return
    const newConfigs = configs.map((c, i) => i === editingIdx ? { ...c, params } : c)
    onChange(newConfigs, dir)
    setOpen(false); setStep('list'); setPicked(null); setEditingIdx(null)
  }

  function remove(idx: number) {
    onChange(configs.filter((_, i) => i !== idx), dir)
  }

  return (
    <div className="flex flex-col gap-1.5">
      {/* Direction selector — shown only when at least one signal is configured */}
      {configs.length > 0 && (
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-[10px] text-slate-500 shrink-0">Направление:</span>
          <div className="flex gap-0.5">
            <button
              type="button"
              onClick={() => onChange(configs, 'buy')}
              title="Срабатывает на Buy-сигнал"
              className={`px-2.5 py-0.5 rounded-l-[4px] text-[10px] font-semibold transition-colors border-y border-l ${
                dir === 'buy'
                  ? 'bg-emerald-700 text-white border-emerald-600'
                  : 'bg-black/20 text-slate-500 hover:text-slate-300 border-white/[.08]'
              }`}
            >Buy</button>
            <button
              type="button"
              onClick={() => onChange(configs, 'sell')}
              title="Срабатывает на Sell-сигнал"
              className={`px-2.5 py-0.5 text-[10px] font-semibold transition-colors border-y ${
                dir === 'sell'
            ...
            <button
              type="button"
              onClick={() => onChange(configs, null)}
              title="Авто: для лонга — Sell, для шорта — Buy"
              className={`px-2.5 py-0.5 rounded-r-[4px] text-[10px] font-semibold transition-colors border-y border-r ${
                dir === null
                  ? 'bg-[#5b8cff]/70 text-white border-[#5b8cff]/50'
                  : 'bg-black/20 text-slate-500 hover:text-slate-300 border-white/[.08]'
              }`}
            >Авто</button>
          </div>
      ...
      {configs.map((sc, idx) => {
        ...
        return (
          <div key={idx} className="flex items-center gap-2 min-w-0">
            <div
              className="flex-1 flex items-center gap-2.5 px-3 py-2 rounded-[9px] min-w-0"
              style={{ background: 'rgba(91,140,255,.06)', border: '1px solid rgba(91,140,255,.18)' }}
            >
              {sig && (
                <span className="shrink-0 w-8 h-5 rounded-[4px] bg-[#1a2545] border border-[rgba(91,140,255,.3)] text-[#8babff] font-bold text-[9px] flex items-center justify-center tracking-[.3px]">
                  {sig.abbr}
                </span>
              )}
              ...
              {tf && (
                <span className="shrink-0 inline-flex items-center gap-[3px] text-[11px] font-mono">
                  <span className="text-[#5b6479]">TF</span>
                  <span className="text-[#8babff] font-semibold">{tf}</span>
                </span>
              )}
          ...
          <div ref={dropRef} style={dropStyle} className="bg-[#0d1628] border border-white/[.10] rounded-xl shadow-2xl overflow-hidden">
            {step === 'list' ? (
              ...
              <div className="p-3 space-y-3">
                ...
                {picked.params.filter((p: { key: string }) => p.key !== 'tf').map((p: { key: string; label: string; kind: string; step?: number; options?: string[] }) => {
                  const val = params[p.key]
                  return (
                    <div key={p.key}>
                      <div className="text-[10px] text-slate-500 mb-1.5">{p.label}</div>
                      {p.kind === 'number' ? (
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {p.options.map((opt: string) => (
                            ...
                            >
                              {({ trend:'Тренд', flat:'Боковик', bull:'Лонг', bear:'Шорт', both:'Оба' } as Record<string,string>)[opt] ?? opt}
                            </button>
                          ))}
                        </div>
                      ) : null}
