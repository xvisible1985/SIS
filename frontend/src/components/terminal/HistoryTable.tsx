...
export function HistoryTable({ accountId, symbol }: Props) {
  ...
  if (!orders.length) return <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400 gap-2"><span className="text-2xl opacity-30">📭</span><p className="text-sm">История пуста</p></div>
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {orders.map(o => (
              <tr key={o.id} className="border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-800/60">
                <td className="px-3 py-2 font-mono text-gray-500 dark:text-gray-400 whitespace-nowrap">{new Date(o.created_at).toLocaleString('ru-RU')}</td>
                ...
                <td className="px-3 py-2 text-gray-500 dark:text-gray-400">{o.order_type}</td>
                ...
                <td className="px-3 py-2 text-right font-mono text-gray-500 dark:text-gray-400">{o.cum_exec_fee}</td>
                <td className="px-3 py-2">
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{o.status}</span>
                </td>
                <td className="px-3 py-2">
