      ...
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        {['all', 'Trade', 'Funding', 'Fee'].map(t => (
          <button key={t} onClick={() => { setType(t); setPage(1) }}
            className={`text-xs px-2 py-0.5 rounded transition-colors ${type === t ? 'bg-blue-500/20 text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>
            {t === 'all' ? 'Все' : t}
          </button>
        ))}
      </div>
      {loading
        ? <div className="flex items-center justify-center flex-1 text-gray-500 dark:text-gray-400 text-sm"><span className="animate-pulse">Загрузка...</span></div>
        : !execs.length
          ? <div className="flex items-center justify-center flex-1 text-gray-500 dark:text-gray-400 text-sm">Нет данных</div>
          : (
                ...
                <tbody>
                  {execs.map(e => (
                    <tr key={e.id} className="border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-800/60">
                      <td className="px-3 py-2 font-mono text-gray-500 dark:text-gray-400 whitespace-nowrap">{new Date(e.exec_time).toLocaleString('ru-RU')}</td>
                      <td className="px-3 py-2 font-mono font-medium">{e.symbol}</td>
                      <td className="px-3 py-2">
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{e.exec_type}</span>
                      </td>
                      <td className="px-3 py-2">
                      ...
                      <td className="px-3 py-2 text-right font-mono text-gray-500 dark:text-gray-400">{e.exec_fee ? parseFloat(e.exec_fee).toFixed(6) : '—'}</td>
      ...
      {total > 100 && (
        <div className="flex items-center justify-between px-3 py-2 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">
          ...
          <div className="flex gap-1">
            ...
            <button onClick={() => setPage(p => p + 1)} disabled={page * 100 >= total} className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 disabled:opacity-40">→</button>
          </div>
        </div>
      )}
    </div>
