...
// Webhooks — alerts on a pkg/signal catalog signal for a chosen symbol/timeframe/params.
// `url` is generated by SIS itself (an internal relay endpoint), never user-supplied — see
// services/api-gateway/webhooks_engine.go.
export interface Webhook {
  id: string
  catalog_signal_id: string | null
  custom_signal_id: string | null
  catalog_signal_name: string
  symbol: string
  timeframe: string
...
export interface CustomSignalComponent {
  signal_id: string
  signal_name: string
  params: Record<string, unknown>
}
...
export interface ExchangeAccount {
  ...
  // Master on/off switch for the whole risk guard above — when false, none of the three
  // thresholds are enforced (values stay stored).
  ...
  // Dashboard's "Очистить статистику" marker — non-destructive, trade_history rows before
  // this timestamp are hidden from the dashboard for this account only.
  stats_cleared_at?: string
}
